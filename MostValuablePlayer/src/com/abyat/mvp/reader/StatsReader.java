...
public class StatsReader {
	 ...
	 * @throws Exception in case of unsupported sport name, incorrect format, or i/o
	 *                   error.
	 */
	public static List<PlayerMatchStats> readMatchStats(InputStream inputStream) throws Exception {
		...
		Class<PlayerMatchStats> playerMatchStatsClass = sport.getPlayerMatchStatsClass();
		...
		while ((inputString = bufferedReader.readLine()) != null) {
			PlayerMatchStats playerMatchStats = playerMatchStatsClass.getConstructor(String.class, Boolean.class)
		...
		for (PlayerMatchStats playerMatchStats : playerMatchStatsList) {
			...
			teamPoints.put(playerMatchStats.getTeamName(),
					sport.calculateTeamScore(playerMatchStats.getTeamName(), playerMatchStatsList));
		}
		...
		Map.Entry<String, Integer> teamWon = teamPoints.entrySet().stream()
				.sorted(Map.Entry.<String, Integer>comparingByValue().reversed()).limit(1).findFirst().get();
